// LEGACY map-sync scheduler — the pre-rewrite implementation, kept behind
// the FLAG_MAP_SERIALIZED_SYNC flag for a staged rollout. Delete once the new
// scheduler in state-updates.ts is at 100%. Do not add features here.
//
// Frozen fork, with one deliberate exception: setSource() went synchronous in
// the rewrite, so calls here use the engine's setSourceAsync() to keep the
// original wait-for-idle behavior the loading indicator depends on.
...
export const useMapStateUpdatesLegacy = (map: MapEngine | null) => {
...
const updateIconsSource = withDebugInstrumentation(
  async (
    ...
    await map.setSourceAsync("icons", features);
  },
  {
...
const rebuildSources = withDebugInstrumentation(
  async (
    ...
    await map.setSourceAsync(FeatureSources.MAIN, features);
    await map.setSourceAsync(FeatureSources.DELTA, []);
...
const updateDeltaSource = withDebugInstrumentation(
  async (
    ...
    await map.setSourceAsync(FeatureSources.DELTA, features);
  },
...
const updateSelection = withDebugInstrumentation(
  async (
    ...
    await map.setSourceAsync("selected-features", features);
  },
...
const updateEphemeralStateSource = withDebugInstrumentation(
  async (
    ...
    await map.setSourceAsync("ephemeral", features);
  },
  {
...
const updateHighlightsSource = async (
  ...
  await map.setSourceAsync("highlights", features);
};
