...
- **New tab, no return urls**: `startCheckoutInBillingApp` opens billing with `window.open(url, "_blank", "noopener,noreferrer")` rather than navigating, so a user who upgrades mid-session keeps their unsaved work in the tab they were editing in. It follows that `buildBillingCheckoutUrl` sends no return url - one would land in the popup rather than the tab the user cares about, opening a second copy of the editor. Billing has no return-url parameters at all any more: it finishes every customer on its own `/checkout/completed` or `/checkout/cancelled` page, which tells them to go back to the tab they started from. It also follows that the billing branch does **not** set `checkoutLoadingAtom`: the current tab never leaves, so a loading dialog would simply never clear.
- **Dependencies**: `NEXT_PUBLIC_BILLING_URL` (defaults to `https://billing.epanetjs.com`). The URL contract - `?plan=&paymentType=&locale=`, and nothing else - lives in `private/apps/billing`. Note the billing app does not post the Slack upgrade alert yet - that moves in spec slice 2, so flag-on upgrades are silent in Slack until then.
- **Locale rides on the same URL**: `buildBillingCheckoutUrl` appends `&locale=` from `useLocale()`, the same way `buildIframeSrc` feeds model-build (`src/dialogs/model-builder-v2-iframe.tsx`). Billing maps it onto the Checkout Session's Stripe `locale`, so the hosted Stripe page matches the language the user reads the app in rather than whatever their browser reports. Reading it from `useLocale()` rather than having billing look up Clerk server-side is deliberate: an anonymous user's locale lives in localStorage, not on a Clerk user, and checkout can start from the landing page with no app session. Billing treats the param as optional and falls back to Stripe's `auto`, so a tab opened before this shipped still works. Billing also carries the locale onto its own pages - `/checkout/completed`, `/checkout/cancelled` and every error page render in the same language, served from a `billing.json` namespace on the locales CDN with English as the fallback. Nothing on the app side is needed for that beyond sending the param.
