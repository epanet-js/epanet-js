...
- **Dependencies**: `NEXT_PUBLIC_BILLING_URL` (defaults to `https://billing.epanetjs.com`). The URL contract - `?plan=&paymentType=&successUrl=&cancelUrl=` - lives in `private/apps/billing`, but the two destinations are built here in `buildBillingCheckoutUrl`: `${window.location.origin}/?notification=checkoutSuccess` and `${window.location.origin}/?dialog=upgrade`. Billing validates both origins against its allowlist and passes them through untouched, so `notification=checkoutSuccess` and `dialog=upgrade` now live in the same repo as the code that reads them (`notification-from-url.tsx` and `dialogFromUrl`). **Both** origins must be on that allowlist, so previews on `*.vercel.app` are rejected by design. Note the billing app does not post the Slack upgrade alert yet - that moves in spec slice 2, so flag-on upgrades are silent in Slack until then.
