...
**A scan belongs to neither half, so it is shared rather than written twice.** `scan-source.ts`, at the package root, parses and reports what the parse found — it answers "what is in this file" without interpreting a single record, so every importer's `scanSource` is that one function. An importer that finds itself wanting its own is interpreting in order to survey, which belongs in `importFromFeatures`. The root is where anything both halves need lives; a folder still never reaches into another's.
...
- **Name what the data was authored in through `sourceProjection`, and only there.** Once the coordinates have moved it is the only trace of where they came from; a scan reports it beside the summary rather than inside one, because it is a fact about the parse and not about the records. It is absent when the records were already WGS84 — a WGS84 `.prj` names nothing, because the file was already there, and a bundle with no `.prj` at all names nothing either, because nobody ever said.
...
- **A drawing has no attribute table, so its layer and its block attributes are the attributes.** `Layer` on every record, `BlockName` on a record that came from a block, and one attribute per block attribute tag. The file's own names win over a tag that repeats them.
- **A block is a symbol unless its name says it was never named.** An anonymous block (`*…`) or one made by pasting (`A$C…`) holds drawing, so it is opened up and its contents become records; any other block is a symbol placed many times, and each placement is one point.
- **A shape a drawing closed is an area; a fill drawn over one is not.** A closed polyline and a circle come back as polygons, while hatches are left out — a hatch is usually decoration over a boundary that is already a record of its own.
- **A drawing can state where it is, and it is believed only over the ground it was drawn for.** Geographic data names a projection the way a `.prj` does, and carries the mesh of control points it was built from; a drawing whose coordinates fall outside that mesh is refused as a mismatch rather than placed, because the claim was written for other ground — a stale one otherwise lands a whole network in the wrong country with coordinates that pass for degrees.
- **A drawing is only as big as its units say.** Millimetres, centimetres and decimetres are scaled to metres; every other unit is left as written, because nothing else is reliably a length a projection expects.
...
type Importer<Role extends string> = {
  ...
  extensions: string[];
  roles: readonly Role[];
  scanSource(input: GisInput): Promise<{ contents: SourceContents | null; sourceProjection?: Proj4Projection; issues: Issue[] }>;
...
**Scanning and importing are separate calls**, because the user chooses a mapping in between and chooses it against what the file turned out to contain. `scanSource` answers "what is in this file" — a `SourceContents` of `attributes`, `recordCount` and `groups`, plus the `sourceProjection` the parse converted out of — without knowing what any of it means, because nothing has told it yet. `importFromSource` then applies the choice, and a preview is the same call with a `recordLimit`.
...
**Both phases take `GisInput` — the files, plus the CRS to read them in, which replaces any they state, and the projection definitions to resolve one against — and a scan returns a summary rather than a handle.** The two are separate because they answer separate questions: `crs` is which projection this particular file is in, `projections` is which projections we can handle at all. A consumer that supports more of them extends the map without touching a mapping, and both phases need them because a scan reprojects exactly as an import does. **Contents are null only when there was nothing to describe**, never merely because an error was raised: a file nobody could place still has attributes and a record count, and a consumer that means to place it itself needs them to build a mapping. Whether to proceed is read from the issues, in the scan exactly as in the import. Both phases still take the same input rather than a handle passed between them: that keeps the contract to one shape, matches `Converter`, and means a consumer that already has the files needs nothing else to parse them.
...
Everything is stated in **records** and **attributes** rather than rows or features, because a source that is not geographic still has a record count, an attribute list and a mapping. `groups` is where the geometric half of that lives: which kinds the source holds, and the records of each, since a consumer that means to treat a file's lines differently from its points needs both. A source of a single kind is not split — its one group is the source — so reading a group in place of the records it came from changes nothing for the overwhelming majority of files.

**A parse works its contents out on first ask, not on the way out.** They belong to the parse — what a file holds is what the bytes said, and answering it twice for one decode would walk every record again — but an import interprets the records itself and never asks, so an eager pass would put one unyielding walk over every property in front of the batching that exists to keep a large import off the main thread. `ParsedGisSource.contents` is computed on first read and kept with the decode, which is the same caching licence the rest of this half has.
