...
- **A supplied CRS stands in for one the file never stated, and never overrides one.** A caller that knows what its export is in says so, and the parser then treats it exactly as the file having said it, reprojection included. `{ type: "unknown" }` says nothing at all, and a bundle *with* a `.prj` ignores it outright, because that `.prj` is the file stating one.
