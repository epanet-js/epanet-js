...
import { replaceNodeWithActiveTopology } from "./replace-node-with-active-topology";
import { NodeAsset, LinkAsset } from "src/hydraulic-model/asset-types";

describe("replaceNodeWithActiveTopology", () => {
  it("replaces junction with tank and preserves connections", () => {
    ...
    const moment = replaceNodeWithActiveTopology(model, {
      oldNodeId: IDS.J1,
      newNodeType: "tank",
    });
  ...
  it("throws error for invalid node ID", () => {
    ...
    expect(() =>
      replaceNodeWithActiveTopology(model, {
