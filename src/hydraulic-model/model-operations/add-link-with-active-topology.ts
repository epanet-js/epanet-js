...
const handlePipeSplits = ({
  ...
  if (startPipeId && endPipeId && startPipeId === endPipeId) {
    ...
    const splitResult = splitPipeWithActiveTopology(hydraulicModel, {
      pipe,
    ...
    if (startPipeId) {
      ...
      const startPipeSplitResult = splitPipeWithActiveTopology(hydraulicModel, {
        pipe: startPipe,
        splits: [startNode],
      });
    ...
    if (endPipeId) {
      ...
      const endPipeSplitResult = splitPipeWithActiveTopology(hydraulicModel, {
        pipe: endPipe,
        splits: [endNode],
      });
