import { splitPipeWithActiveTopology } from "./split-pipe-with-active-topology";
import {
...
describe("splitPipeWithActiveTopology", () => {
  it("splits a pipe at specified coordinates", () => {
    ...
    const { putAssets, deleteAssets } = splitPipeWithActiveTopology(
      hydraulicModel,
      {
        pipe,
        splits: [splitNode],
  ...
  it("generates correct labels for split pipes", () => {
    ...
    const { putAssets } = splitPipeWithActiveTopology(hydraulicModel, {
      pipe,
      splits: [splitNode],
    });
  ...
  it("throws error when no splits provided", () => {
    ...
    expect(() => {
      splitPipeWithActiveTopology(hydraulicModel, {
        pipe,
        splits: [],
      });
  ...
  it("reconnects customer points to appropriate split segments", () => {
    ...
    const { putAssets, putCustomerPoints } = splitPipeWithActiveTopology(
      hydraulicModel,
      {
        pipe,
        splits: [splitNode],
  ...
  it("preserves isActive when splitting active pipe", () => {
    ...
    const moment = splitPipeWithActiveTopology(model, {
      pipe,
      splits: [splitNode],
    });
