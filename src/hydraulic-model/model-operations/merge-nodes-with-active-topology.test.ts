...
import { mergeNodesWithActiveTopology } from "./merge-nodes-with-active-topology";
...
describe("mergeNodesWithActiveTopology", () => {
  it("merges J1 into J2 position with J1 surviving", () => {
    ...
    const moment = mergeNodesWithActiveTopology(model, {
      sourceNodeId: IDS.J1,
      targetNodeId: IDS.J2,
    });
  ...
  it("throws error for invalid source node ID", () => {
    ...
    expect(() => {
      mergeNodesWithActiveTopology(model, {
        sourceNodeId: 999,
        targetNodeId: IDS.J1,
      });
