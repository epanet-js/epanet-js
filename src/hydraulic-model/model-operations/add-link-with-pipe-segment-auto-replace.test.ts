...
import { addLinkWithPipeSegmentAutoReplace } from "./add-link-with-pipe-segment-auto-replace";
import {
...
describe("addLinkWithPipeSegmentAutoReplace", () => {
  describe("basic functionality (no pipe splitting)", () => {
    it("updates connections", () => {
      ...
      const { putAssets } = addLinkWithPipeSegmentAutoReplace(hydraulicModel, {
        startNode,
        endNode,
        link,
      });
  ...
  describe("pipe splitting functionality", () => {
    it("splits start pipe when startPipeId provided", () => {
      ...
      const { putAssets, deleteAssets } = addLinkWithPipeSegmentAutoReplace(
        hydraulicModel,
        {
          startNode,
          endNode,
    ...
    it("reconnects customer points when splitting start pipe", () => {
      ...
      const { putAssets, putCustomerPoints } =
        addLinkWithPipeSegmentAutoReplace(hydraulicModel, {
          startNode,
          endNode,
          link,
          startPipeId: IDS.P1,
  ...
  describe("with overlapping pipe section", () => {
    ...
    it("reallocates customer points to all pipes including new pipe when drawing pipe", () => {
      ...
      const { putCustomerPoints } = addLinkWithPipeSegmentAutoReplace(
        hydraulicModel,
        {
          startNode,
          endNode,
