...
import { replaceLinkWithPipeSegmentAutoReplace } from "./replace-link-with-pipe-segment-auto-replace";
import {
...
describe("replaceLinkWithPipeSegmentAutoReplace", () => {
  describe("basic functionality", () => {
    it("replaces existing pipe with new pipe", () => {
      ...
      const { putAssets, deleteAssets } = replaceLinkWithPipeSegmentAutoReplace(
        hydraulicModel,
        {
          sourceLinkId: IDS.P1,
          newLink: newPipe,
    ...
    it("throws error for mismatched link types", () => {
      ...
      expect(() =>
        replaceLinkWithPipeSegmentAutoReplace(hydraulicModel, {
          sourceLinkId: IDS.P1,
          newLink: newPump,
          startNode,
          endNode,
  ...
  describe("active topology status inheritance", () => {
    it("inherits isActive from source link when replacing active link", () => {
      ...
      const { putAssets } = replaceLinkWithPipeSegmentAutoReplace(
        hydraulicModel,
        {
          sourceLinkId: IDS.P1,
          newLink: newPipe,
  ...
  describe("customer points reconnection", () => {
    it("reconnects customer points to closest junction", () => {
      ...
      const { putCustomerPoints, putAssets } =
        replaceLinkWithPipeSegmentAutoReplace(hydraulicModel, {
          sourceLinkId: IDS.P1,
          newLink: newPipe,
          startNode,
    ...
    it("recalculates snap point when new pipe has different geometry", () => {
      ...
      const { putCustomerPoints } = replaceLinkWithPipeSegmentAutoReplace(
        hydraulicModel,
        {
          sourceLinkId: IDS.P1,
          newLink: newPipe,
