...
import { readInpDataWithControls } from "./read-inp-data-with-controls";
import { buildModelWithControls } from "./build-model-with-controls";
...
export const parseInpWithControls = (
  inp: string,
  ...
  const { inpData, stats } = readInpDataWithControls(inp, issues, safeOptions);
  const { hydraulicModel, modelMetadata } = buildModelWithControls(
    inpData,
    issues,
    safeOptions,
  );
