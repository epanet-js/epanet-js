...
import { buildModelWithAllCurves } from "./build-model-with-all-curves";
...
export const parseInpWithAllCurves = (
  inp: string,
  ...
  const { hydraulicModel, modelMetadata } = buildModelWithAllCurves(
    inpData,
    issues,
    safeOptions,
  );
