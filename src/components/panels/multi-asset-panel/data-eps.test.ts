...
describe("computeMultiAssetDataEps", () => {
  ...
  it("groups assets by type", () => {
    ...
    const result = computeMultiAssetDataEps(assets, quantities, hydraulicModel);
  ...
  it("handles empty asset arrays", () => {
    ...
    const result = computeMultiAssetDataEps([], quantities, hydraulicModel);
  ...
  describe("average demand calculation", () => {
    it("calculates average demand for junction with only constant demands", () => {
      ...
      const assets = Array.from(hydraulicModel.assets.values());
      const result = computeMultiAssetDataEps(
        assets,
        quantities,
        hydraulicModel,
      );
