...
export const AssetDataTableWithFeatures = memo(
  function AssetDataTableWithFeaturesInner({ assetType }: AssetDataTableProps) {
    const dataGridRef = useRef<DataGridWithFeaturesRef>(null);
    ...
    const onChange = useCallback(
      (newRows: AssetRow[]) => {
        ...
        for (let i = 0; i < newRows.length; i++) {
          ...
          if (assetType === "valve") {
            const kindChangeIdx = changes.findIndex(
              (c) => c.property === "kind",
            );
            if (kindChangeIdx !== -1) {
