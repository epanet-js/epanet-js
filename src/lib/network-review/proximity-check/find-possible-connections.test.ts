...
describe("findPossibleConnections", () => {
  it("finds nodes near pipes they are not connected to", () => {
    ...
    const possibleConnections = findPossibleConnections(data, 50);

    expect(possibleConnections).toHaveLength(1);
    expect(idsLookup[possibleConnections[0].nodeId]).toEqual("J3");
    expect(idsLookup[possibleConnections[0].connection.pipeId]).toEqual("P1");
    expect(possibleConnections[0].connection.distance).toBeLessThan(50);
  });
  ...
  it("does not report nodes that are too far from any pipe", () => {
    ...
    const possibleConnections = findPossibleConnections(data, 0.5);

    expect(possibleConnections).toHaveLength(0);
  });

  it("finds multiple nodes near the same pipe", () => {
    const model = HydraulicModelBuilder.with()
      .aJunction("J1", { coordinates: [0, 0] })
    ...
    const nodeIds = possibleConnections.map((pc) => idsLookup[pc.nodeId]);
  ...
  it("chooses the nearest pipe when multiple pipes are candidates", () => {
    ...
    expect(possibleConnections).toHaveLength(1);
    expect(idsLookup[possibleConnections[0].nodeId]).toEqual("J5");
    expect(idsLookup[possibleConnections[0].connection.pipeId]).toEqual("P1");
  });
  ...
  it("does not suggest connections too close to already connected junctions", () => {
    ...
    const possibleConnections = findPossibleConnections(data, 50, 5);
  ...
  it("handles nodes connected to valves and pumps", () => {
    ...
    expect(possibleConnections).toHaveLength(1);
    expect(idsLookup[possibleConnections[0].nodeId]).toEqual("J4");
    expect(idsLookup[possibleConnections[0].connection.pipeId]).toEqual("P1");
  });
});
