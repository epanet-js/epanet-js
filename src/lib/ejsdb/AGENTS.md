...
- Editing bulk-insert column lists in `worker-api.ts` (they must match the table schema exactly)
- Editing row-shape Zod schemas in `schema/*.ts` (they must match the columns)
...
3. Update the matching Zod schema in `schema/*.ts`
4. Update consuming mapping code in `src/lib/db/mappers/**` and any column lists in `worker-api.ts`
